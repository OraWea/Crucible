...
class LLMCore:
    ...
    def extract_concepts(self, raw_input_text: str) -> List[Dict[str, Any]]:
        ...
        system_prompt = "你是一个专业的知识图谱抽取专家。你擅长从凌乱的文本、音频转录和图像描述中识别出核心学术概念。"
        
        prompt = f"""请分析以下非结构化文本，提取出其中讨论的所有核心学术名词、概念、算法或技术点。
对于每个关键概念，必须提取出其详细释义、包含的要点（Key Points），以及涉及的任何代码块或 LaTeX 公式。
...
[
  {{
    "concept": "概念/名词名称（必须简短、精准，例如 'Transformer'）",
    "definition": "对该概念在文中的详细中文定义与原理解释",
    "key_points": ["核心要点1", "核心要点2", "核心要点3"],
    "code_or_formula": "提取的相关代码块或 LaTeX 数学公式（若无则填空字符串）"
  }}
]
"""
        raw_response = ""
