...
from utils.fs_router import fs_router
from utils.wiki_editor import wiki_editor
...
class LLMCore:
    ...
    使用 LLMManager 统一调度，支持多Provider配置
    """

    def __init__(self):
        self._module_name = "concept_extractor"
    ...
    def extract_concepts(self, raw_input_text: str) -> List[Dict[str, Any]]:
        ...
        prompt = """请分析以下非结构化文本，提取出其中讨论的所有核心主题、概念、人物、专有名词或关键知识点。
...
""".format(raw_input_text=raw_input_text)
...
"""
        ...
        prompt = """你的任务是将新提取的关于"{concept_name}"的知识，无缝融合合并到它已有的旧笔记中。
...
""".format(
            ...
            new_knowledge_summary=new_knowledge_summary,
            old_content=old_content if old_content else "（无旧笔记，请从头开始创建）",
            timestamp=timestamp,
