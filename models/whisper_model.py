...
class WhisperTranscriber:
    ...
    def transcribe(self, audio_path: str, language: str = "auto") -> List[Dict[str, Any]]:
        """
        使用百炼实时流式 ASR 模型对本地音频文件进行转写，输出带精准时间戳的片段列表。
        
        通过 Recognition 类将本地 WAV 文件的裸 PCM 数据以流式方式分片发送至百炼服务端。
        
        Args:
            audio_path: 本地已转换的 WAV 音频路径 (16kHz, 单声道, 16bit)
